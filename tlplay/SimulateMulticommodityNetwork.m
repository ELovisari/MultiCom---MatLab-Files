        ...
        if numel(aiConsecutiveEdges) > 0
            ...
            for i = 1:nof
                % ComputeG(sum(afRhoOfCurrentEdges)', afCurrentThreshold, afCurrentAlphaRouting(:, i), afCurrentBetaRouting(:, i), 0)
                   G(:,i) = ComputeG(sum(afRhoOfCurrentEdges)', afCurrentThreshold, afCurrentAlphaRouting(:, i), afCurrentBetaRouting(:, i), 0);
            end
            totrho = sum(afRhoOfCurrentEdges)';
            
           bp = totrho(1)*exp(-totrho(1))/(totrho(1)*exp(-totrho(1))+ exp(-sum(totrho(2:end))));
            %bp = exp(-1*sum(afRhoOfCurrentEdges(:,1)));
            for i = 1:nof
